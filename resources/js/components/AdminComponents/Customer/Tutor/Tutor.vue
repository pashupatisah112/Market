...
<v-container fluid>
    ...
    <v-row>
        <v-data-table :headers="headers" :items="tutors" class="elevation-1">
            <template v-slot:top>
                <v-toolbar flat color="white">
                    ...
                    <v-toolbar-title>tutor Management</v-toolbar-title>
                        ...
                        <v-card>
                            ...
                            <v-card-text>
                                <v-container>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field v-model="editedItem.name" label="Tutor Name" :rules="nameRules"></v-text-field>
...
export default {
    ...
    computed: {
        formTitle() {
            return this.editedIndex === -1 ? 'New tutor' : 'Edit tutor Details'
        },
    ...
    methods: {
        initialize() {
            ...
            then(res => this.tutors = res.data.tutors)
        ...
        deleteItem(item) {
            const index = this.tutors.indexOf(item)
            confirm('Are you sure you want to delete this item?') && this.tutors.splice(index, 1)
            axios.delete('/api/tutors/' + item.id)
        ...
        save() {
            if (this.editedIndex > -1) {
                axios.put('/api/tutors/' + this.editedItem.id, {
                        'name': this.editedItem.name,
                        'email': this.editedItem.email,
                        'password': this.editedItem.email,
                    })
                    .then(res => Object.assign(this.tutors[this.editedIndex], res.data.tutors))
                    .catch(err => console.log(err.response))
                Object.assign(this.tutors[this.editedIndex], this.editedItem)
            } else {
                axios.post('/api/tutors', {
                        'name': this.editedItem.name,
                        'email': this.editedItem.email,
                        'password': this.editedItem.password
                    })
                    .then(res => this.tutors.push(res.data.tutors))
